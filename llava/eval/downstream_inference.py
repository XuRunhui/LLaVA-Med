...
def eval_model(args):
    ...
    if args.mm_projector is None:
        ...
        if "BiomedCLIP" in model_name or "biomed_clip" in model_name:
            # FIXED: Added low_cpu_mem_usage=True
            ...
            # FIXED: Added low_cpu_mem_usage=True to handle large sharded checkpoints
            model = LlavaLlamaForCausalLM.from_pretrained(
                model_name,
                torch_dtype=torch.float16,
                use_cache=True,
                low_cpu_mem_usage=True  # ← CRITICAL FIX for sharded checkpoints
            ).cuda()
            image_processor = CLIPImageProcessor.from_pretrained(model.config.mm_vision_tower, torch_dtype=torch.float16)

            # FIXED: Vision tower may be meta tensor - reload from pretrained
            vision_tower = model.model.vision_tower[0]
            if hasattr(vision_tower, 'is_meta') or not next(vision_tower.parameters()).is_cuda:
                # Vision tower not properly loaded - reload it
    ...
    for i, line in enumerate(tqdm(questions)):
        ...
        # FIXED: Use deterministic generation (temperature=0 or do_sample=False)
        with torch.inference_mode():
            output_ids = model.generate(
                input_ids,
                images=images,
                do_sample=False,  # FIXED: Changed from True to False for deterministic results
                temperature=None,  # FIXED: Set to None when do_sample=False
                max_new_tokens=1024,
        ...
        if args.answer_prompter:
            ...
            with torch.inference_mode():
                output_ids = model.generate(
                    input_ids,
                    images=images,
                    do_sample=False,  # FIXED: Deterministic
                    temperature=None,
                    max_new_tokens=64,
                    stopping_criteria=[stopping_criteria])
